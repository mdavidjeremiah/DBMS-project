...
export function DataNotice({ error, rlsBlocked }: { error: string | null; rlsBlocked: boolean }) {
  ...
  if (rlsBlocked) return <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-950/30 dark:text-amber-300"><ShieldAlert className="mt-0.5 size-4 shrink-0" />Your FastAPI role does not currently permit this view.</div>
  return null
}
